...
- **Client tests**: `client/src/**/*.test.{js,jsx}` — run in `jsdom` environment using Testing Library + `@testing-library/user-event`.
- **Server tests**: `server/src/**/*.test.js` — run in `node` environment using Supertest.
...
- **Client**: Vite handles module resolution — no import extensions needed.
...
- **New API routes**: Add a router file under `server/src/routers/`. Mount it in `index.js`. Include complete Swagger JSDoc annotations.
...
- Server relative imports **must** use `.js` extensions (ES Module resolution).
...
Multi-stage Dockerfile: build stage builds client via Vite, production stage copies `server/src` + `server/public` with production deps. Exposes port 8080.
